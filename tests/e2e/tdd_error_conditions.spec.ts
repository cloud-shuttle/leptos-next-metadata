...
test.describe('TDD Error Conditions & Failure Scenarios', () => {
  ...
  test('should detect duplicate metadata entries', async ({ page }) => {
    ...
    const duplicates = Array.from(metaMap.entries()).filter(([_, count]) => count > 1);
    
    // Allow some duplicates for different contexts (e.g., og:title and twitter:title)
    const allowedDuplicates = ['title', 'description'];
    const criticalDuplicates = duplicates.filter(([key, _]) => !allowedDuplicates.includes(key));
    
    if (criticalDuplicates.length > 0) {
      console.log('⚠️  Warning: Duplicate metadata detected:', criticalDuplicates);
    }
  ...
  test('should handle missing critical metadata gracefully', async ({ page }) => {
    // Given: A test page is loaded
    await page.goto('/', { waitUntil: 'domcontentloaded', timeout: 10000 });
  ...
  test('should validate metadata consistency across platforms', async ({ page }) => {
    // Given: A test page is loaded
    await page.goto('/', { waitUntil: 'domcontentloaded', timeout: 10000 });
